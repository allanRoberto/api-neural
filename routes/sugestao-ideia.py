...
from patterns.chain import PatternChain
...
def calcular_ensemble(
    ...
    print(temporal_candidates, "candidatos temporais")

    print(resultado_estelar.scores.items(), "candidatos estelar")
    print(resultado_chain.scores.items(), "candidatos chain")
    print(resultado_master.scores.items(), "candidatos estelar")
...
def compute_consensus_score_for_number(
    numero: int,
    master_nums: Set[int],
    estelar_nums: Set[int],
    chain_nums: Set[int],
    temporal_nums: Set[int],
) -> int:
    """
    Retorna quantos padrões sugeriram esse número (0 a 5).
    """
    count = 0
    if numero in master_nums:
        count += 1
    if numero in estelar_nums:
        count += 1
    if numero in chain_nums:
        count += 1
    if numero in temporal_nums:
        count += 1
    return count
...
def compute_confidence_index(
    ...
    """
    Índice de confiança (0..1) baseado em:
      - consenso entre padrões,
      - concentração do peso nos top N,
      - suporte amostral (total_spins),
      - estabilidade temporal (dias com ocorrência).
    """

    if not scores_ensemble:
        return 0.0

    # 1) Consenso médio dos números (0..1)
    consenso_total = 0.0
    for n in scores_ensemble.keys():
        consenso_total += compute_consensus_score_for_number(
            n,
            master_nums,
            estelar_nums,
            chain_nums,
    ...
    # 5 padrões no máximo
    C_consenso = consenso_total / (len(scores_ensemble) * 5.0)

    # 2) Concentração (peso top5 / peso total)
    pesos = sorted(scores_ensemble.values(), reverse=True)
    top5_sum = sum(pesos[:5])
    total_sum = sum(pesos) if pesos else 1.0
    C_conc = top5_sum / total_sum

    # 3) Suporte amostral (total_spins vs alvo)
    alvo_spins = 80.0  # você pode ajustar esse alvo
    C_suporte = min(total_spins / alvo_spins, 1.0) if total_spins > 0 else 0.0

    # 4) Estabilidade temporal (dias que teve ocorrência / days_back)
    dias_com_ocorrencia = temporal_metadata.get("days_with_occurrences", 0)
    if days_back > 0:
        C_estab = min(dias_com_ocorrencia / days_back, 1.0)
    else:
        C_estab = 0.0

    # 5) Aderência a região (por enquanto um valor fixo; depois dá pra refinar)
    C_regiao = 0.7

    # Pesos de cada componente (pode tunar depois)
    ...
    # Garante [0,1] e com 3 casas decimais
...
async def sugestao_ensemble(
    ...
    try:
        ...
        chain = PatternChain(config=config_chain)
        ...
        # conjuntos por padrão (se ainda não tiver feito)
        master_set = set(resultado_master.candidatos)
        estelar_set = set(resultado_estelar.candidatos)
        chain_set = set(resultado_chain.candidatos)
        temporal_set = set(resultado_temporal[0].keys())

        # total de giros usados no temporal (vem do metadata do temporal)
        total_spins_temporal = resultado_temporal[1].get("total_spins", 0)
        temporal_metadata = resultado_temporal[1]

        indice_confianca = compute_confidence_index(
            scores_ensemble=scores_ensemble,
            master_nums=master_set,
            estelar_nums=estelar_set,
            chain_nums=chain_set,
            temporal_nums=temporal_set,
            total_spins=total_spins_temporal,
            days_back=days_back,
